...
{
    /// <summary>
    /// FluentCRM class used to encapsulate access to the DocumentTemplate Entity
    /// </summary>
    public class FluentDocumentTemplate : FluentCRM
    {

        private const string _logicalName = "documenttemplate";

        #region "Constructors etc required by Language"
        private FluentDocumentTemplate(Guid id, IOrganizationService service) : base(_logicalName, id, service) { }

        protected FluentDocumentTemplate(IOrganizationService service) : base(_logicalName, service) { }

        private FluentDocumentTemplate(Guid id) : base(_logicalName, id) { }
        ...
        public static IEntitySet DocumentTemplate(Guid id, IOrganizationService service)
        {
            return new FluentDocumentTemplate(id, service);
        }
        ...
        public static IUnknownEntity DocumentTemplate(IOrganizationService service)
        {
            return new FluentDocumentTemplate(service);
        }
        ...
        /// <returns>FluentCRM subclass - returns even if ID does not exist.</returns>
        public static IEntitySet DocumentTemplate(Guid id)
        {
            return new FluentDocumentTemplate(id);
        }
        ...
        /// </summary>
        /// <returns>FluentCRM subclass that can be used to filter and operate on the specified entity type.</returns>
        public static IUnknownEntity DocumentTemplate()
        {
            return new FluentDocumentTemplate();
        }

        /// <summary>
        /// Parameterless constructor required by the language, but not necessarily used.
        /// </summary>
        public FluentDocumentTemplate() : base(_logicalName) { }

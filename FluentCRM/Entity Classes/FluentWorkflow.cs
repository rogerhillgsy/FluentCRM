...
{
    /// <summary>
    /// FluentCRM class used to encapsulate access to the Workflow Entity
    /// </summary>
    public class FluentWorkflow : FluentCRM
    {

        private const string _logicalName = "workflow";

        #region "Constructors etc required by Language"
        private FluentWorkflow(Guid id, IOrganizationService service) : base(_logicalName, id, service) { }

        /// <summary>
        /// Set IOrganizationService to use
        /// </summary>
        /// <param name="service"></param>
        protected FluentWorkflow(IOrganizationService service) : base(_logicalName, service) { }

        private FluentWorkflow(Guid id) : base(_logicalName, id) { }
        ...
        public static IEntitySet Workflow(Guid id, IOrganizationService service)
        {
            return new FluentWorkflow(id, service);
        }
        ...
        public static IUnknownEntity Workflow(IOrganizationService service)
        {
            return new FluentWorkflow(service);
        }
        ...
        /// <returns>FluentCRM subclass - returns even if ID does not exist.</returns>
        public static IEntitySet Workflow(Guid id)
        {
            return new FluentWorkflow(id);
        }
        ...
        /// </summary>
        /// <returns>FluentCRM subclass that can be used to filter and operate on the specified entity type.</returns>
        public static IUnknownEntity Workflow()
        {
            return new FluentWorkflow();
        }

        /// <summary>
        /// Parameterless constructor required by the language, but not necessarily used.
        /// </summary>
        public FluentWorkflow() : base(_logicalName) { }
